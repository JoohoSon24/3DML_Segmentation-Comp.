...
class ScanNetEval(object):
    ...
    def evaluate_matches(self, matches):
        ...
        # results: class x iou
        ap = np.zeros((len(dist_threshes), len(self.eval_class_labels), len(ious)), dtype=np.float64)
        rc = np.zeros((len(dist_threshes), len(self.eval_class_labels), len(ious)), dtype=np.float64)
